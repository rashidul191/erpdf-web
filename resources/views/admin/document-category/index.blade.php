<x-admin-app-layout>
    <div class="w-full flex justify-between mb-2">
        <div class="text-xl">{{ __('Document Categories') }}</div>
    </div>
    {{-- Create Form Start --}}
                    ...
                    dataSrc(response) {
                        response.data.map(function (item) {
                            item.action = actionIcons({
                                'edit': '{{ route('admin.document-categories.edit', '@') }}'.replace('@', item
                                    .id),
                                'delete': '{{ route('admin.document-categories.destroy', '@') }}'.replace('@',
                                    item.id),
                            });
