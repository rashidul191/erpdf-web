<x-admin-app-layout :title="__('Edit Our Story')">

    <div class="pb-3 flex justify-between">
        <div class="text-md md:text-2xl">{{ __('Edit Our Story') }}</div>
        <div>
    ...
    <form action="{{ route('admin.our-story.update', $ourStory->id) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="bg-white p-4">
            ...
            <div class="flex flex-wrap justify-center w-full">
                ...
                <x-labeled-input label="Year Duration" name="date" value="{{ $ourStory->date }}" class="w-full p-1 md:w-1/2" />
                <x-labeled-input name="title" value="{{ $ourStory->title }}" class="w-full p-1 md:w-1/2" />
                <x-labeled-textarea label="Description" name="description" value="{!! $ourStory->description !!}" class="w-full p-1 md:w-1/2" />
