<x-admin-app-layout :title="__('Edit Blog Category')">

    <div class="pb-3 flex justify-between">
        <div class="text-md md:text-2xl">{{ __('Edit Blog Category') }}</div>
        <div>
            <a class="text-primary-700 font-semibold bg-red-200 py-2 px-3 rounded"
                href="{{ route('admin.blog-categories.index') }}">{{ __('Back') }}</a>
        </div>
    </div>
    ...
    <div class="bg-white p-4 rounded">
        <form action="{{ route('admin.blog-categories.update', $blogCategory->id) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            <img width="50" id="prevImage" src="{{ $blogCategory->image }}">
            <div class="flex flex-wrap justify-center w-full">
                ...
                <x-labeled-input name="name" value="{{ $blogCategory->name }}" class="w-full p-1 md:w-1/2" />
