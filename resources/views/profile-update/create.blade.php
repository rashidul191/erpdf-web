            ...
            <div class="w-full bg-white rounded">
                ...
                <div class="w-full p-4">
                    <form action="{{ route('profile-update.store') }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        <x-labeled-input type="text" class="" name="name" value="{{ old('name', auth()->user()->name) }}" readonly/>
                        <x-labeled-input type="email" class="mt-4" name="email" value="{{ old('email', auth()->user()->email) }}" readonly/>
